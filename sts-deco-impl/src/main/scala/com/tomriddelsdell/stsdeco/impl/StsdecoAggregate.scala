package com.tomriddelsdell.stsdeco.impl
...
/**
  * This provides an event sourced behavior. It has a state, [[StsdecoState]], which
  ...
  */
object StsdecoBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[StsdecoCommand]): Behavior[StsdecoCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, StsdecoEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[StsdecoCommand, StsdecoEvent, StsdecoState](
        persistenceId = persistenceId,
        emptyState = StsdecoState.initial,
...
/**
  ...
  */
case class StsdecoState(message: String, timestamp: String) {
  def applyCommand(cmd: StsdecoCommand): ReplyEffect[StsdecoEvent, StsdecoState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: StsdecoEvent): StsdecoState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[StsdecoEvent, StsdecoState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[StsdecoEvent, StsdecoState] =
    Effect
...
object StsdecoState {
  ...
  /**
    ...
    */
  def initial: StsdecoState = StsdecoState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[StsdecoCommand]("StsdecoAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[StsdecoState] = Json.format
}

/**
  * This interface defines all the events that the StsdecoAggregate supports.
  */
sealed trait StsdecoEvent extends AggregateEvent[StsdecoEvent] {
  def aggregateTag: AggregateEventTag[StsdecoEvent] = StsdecoEvent.Tag
}

object StsdecoEvent {
  val Tag: AggregateEventTag[StsdecoEvent] = AggregateEventTag[StsdecoEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends StsdecoEvent
...
/**
  ...
  */
trait StsdecoCommandSerializable

/**
  * This interface defines all the commands that the StsdecoAggregate supports.
  */
sealed trait StsdecoCommand
    extends StsdecoCommandSerializable
